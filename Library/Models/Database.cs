		...
		using (var dr = cm.ExecuteReader()) {
			while (dr.Read()) {
				...
				if (t != null) {
					...
					if (c != null)
						c.Default = new Default(t, c, (string)dr["DEFAULT_NAME"],
...
";
	...
	public string ScriptCreate() {
		...
		foreach (var t in Tables.Concat(TableTypes)) {
			...
			foreach (var constraint in t.Constraints.Where(
				         c => c.Type == "CHECK" && !c.ScriptInline))
			...
			var defaults = (
					...
					select c.Default)
				.ToArray();
			foreach (var def in defaults) text.AppendLine(def.ScriptCreate());
		}
	...
	public void ScriptToDir(string tableHint = null, Action<TraceLevel, string> log = null) {
		...
		foreach (var table in Tables) {
			WriteScriptDir("check_constraints",
				table.Constraints.Where(c => c.Type == "CHECK").ToArray(), log);
