				...
				using (var dr = cm.ExecuteReader()) {
					while (dr.Read()) {
						foreach (var c in cols) {
							if (dr.GetDataTypeName(dr.GetOrdinal(c.Name)).EndsWith(".sys.geometry"))
								...
								data.Write(StringUtil.ToHexString(dr.GetSqlBytes(dr.GetOrdinal(c.Name)).Value));
							else if (dr[c.Name] is DBNull)
