...
";
		...
		private void LoadConstraintsAndIndexes(SqlCommand cm) {
			...
			var from_sql = new[] {
				"select object_id, name, schema_id, 'T' as baseType from sys.tables",
				"select object_id, name, schema_id, 'V' as baseType from sys.views",
				"select type_table_object_id as object_id, name, schema_id, 'TVT' as baseType from sys.table_types"
			};
			foreach (var from_statement in from_sql)
			{
				try {
					cm.CommandText = @"
						select 
